        ...
        if !actions.is_empty(){
            ...
            if let Some(game) = game_guard.as_mut(){
                for (clientid, actionsstr) in actions{
                    let player_color = game_loop_clients_to_index_ref.lock().unwrap().get(&clientid).cloned();
                    ...
                    if let Some(col) = player_color {
                        if current_color == col {
                            match game.play(&actionsstr) {
                                Ok(()) =>{
                                    println!("Aktion von {} erfolgreich {}", clientid, actionsstr);
                                    state_changed = true;
                                } ,
                                Err(e) => {
...
// Bei erfolgreichem Zug State an alle
if state_changed {
    let state_msg = {

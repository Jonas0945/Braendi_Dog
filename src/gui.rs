...
impl<'a> canvas::Program<Message> for BoardView<'a> {
    ...
    fn update(
        ...
        if let canvas::Event::Mouse(mouse::Event::ButtonPressed(mouse::Button::Left)) = event {
            ...
            for i in 0..total_tiles {
                ...
                if is_hit(cursor_position, pos, 12.0) {
                    // 12px toleranz
                    return (
    ...
    fn draw(
        ...
        let current_color = match self.game.current_player().color {
        ...
        for i in 0..total_tiles {
            ...
            // Default tile color
            let tile_color = match board_state.get(i).and_then(|t| t.as_ref()) {
            ...
            // Draw highlight outline if this tile is a possible move
            if self.highlights.contains(&i) {
                frame.stroke(
                    &circle,
                    canvas::Stroke::default()
                        .with_color(current_color)
                        .with_width(5.0), // Big outline
                );
            }
        }
...
impl<'a> canvas::Program<Message> for HandView<'a> {
    ...
    fn draw(
        ...
        for (_i, card, rect, is_hovered) in layout {
            ...
            let txt_col = if card == Card::Joker {
